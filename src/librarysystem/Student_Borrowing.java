...
public class Student_Borrowing {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Student_Borrowing window = new Student_Borrowing(); 
	...
	private void initialize() {
		...
		frmBookBorrowing.setTitle("Student Borrow Books");
		...
		JLabel lblNewLabel = new JLabel("Student Borrow Books");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				LibraryNavigation ln = new LibraryNavigation();
				ln.setVisible(true);
				frmBookBorrowing.dispose();
			}
		});
		btnNewButton.setBounds(574, 340, 94, 34);
		frmBookBorrowing.getContentPane().add(btnNewButton);
		...
		btnBorrow.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String name = Student_Details.Name();
				String studentID = Student_Details.Student_ID();
				String year_level = Student_Details.year_level();
				String section_ = Student_Details._section();
...
//				success = "Name: \n" + bNameInput.getText() + "\n\nIBSN: \n" + bISBNInput.getText();
				 ...
				 if(btnBorrow.getText().equals("Search Book")) {
						...
						try {
							query = "SELECT COUNT(*) FROM borrowed where name='" + name + "' && role='" + userRole + "'";
							rs = stmt.executeQuery(query);
							if (rs.next()) {
								int count = rs.getInt(1);
								if (count >= 3) {
									JOptionPane.showMessageDialog(frmBookBorrowing, "You Exceed Your Book Borrowing Limit (3 Books Only to Borrow)");
								}
							}
														...
														if (category.equals("Fictional") || category.equals("Non-Fictional")) {
															...
															if (rs3.next()) {
																...
																if (count_status == 1) {
																	...
																	try {
																		query2 = String.format("INSERT INTO borrowed (name, ISBN, date_borrowed, date_due, role) VALUES ('%s', '%s', '%s', '%s', '%s')", name, bISBNInput.getText(), todayAsString, futureDateAsString, userRole);
																		...
																		JOptionPane.showMessageDialog(frmBookBorrowing, "Must Return in: " + futureDateAsString);
																		if (rs2 > 0 && rs4 > 0) {
